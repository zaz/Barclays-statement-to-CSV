...
def barclays_parse(lines)
	return enum_for :barclays_parse, lines unless block_given?
...
def readlines(files)
	return enum_for :readlines, files unless block_given?
...
def check(output)
	return enum_for :check, output unless block_given?

	yield output.first
	output.each_cons(2) do |a, b|
		printf "A "
		yield a
		printf "  "
		yield b
		next
		b[:date] = a[:date] unless b[:date]
		case b[:description]
		when :start
			abort ":start not preceded by :end" unless a[:description] == :end
			abort "start balance seems to be a transaction" if b[:transaction]
			abort "start balance != previous end balance" if b[:balance] != a[:balance]
		when :end
			abort "end balance seems to be a transaction" if b[:transaction]
			abort "end balance != previous balance" if b[:balance] != a[:balance]
		else
			abort "no amount specified for transaction" unless b[:transaction]
			if b[:balance] and a[:balance]
				difference = b[:balance] - a[:balance]
				puts difference
				if b[:transaction] == difference.abs
					b[:transaction] = difference
				else
					abort "transaction doesn't match difference in balance"
				end
			end
		end
	end
...
# barclays_parse(readlines(ARGV)).each_cons(2) do |a, i|
bp = []
barclays_parse(readlines(ARGV)) do |i|
	bp << i
	bp.each do |x|
		print x[:date].to_s[-4,5].sub('-', '') if x[:date]
		print ' ', x[:transaction] if x[:transaction]
		case x[:description]
		when :start; print '!'
		when :end; print '|'
		else; print ')'
...
puts bp.length
exit
# check(barclays_parse(readlines(ARGV))).each_cons(2) do |i|
check(bp).each do |i|
	puts i
	next
	date = i[:date] ||= " " * 10
